...
class SwitchRuntime(app_manager.RyuApp):
    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None):
        ...
        inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,
                                             actions)]
        '''
        Hard time-out: the flow will be removed independently if has <flow match> in rule in a given time
        Idle time-out: the flow will be remove if after some time has no <flow match> in rule in a given time
        '''
        
        if buffer_id:
            mod = parser.OFPFlowMod(datapath=datapath, idle_timeout=10, buffer_id=buffer_id,
    ...
    def switch_reconnected(self, ev):
        #PathManager.update_topology(self)
        lsp = PathManager("","","","","")
    ...
    def host_connected(self, ev):
        print("Host Connectado")
        
    @set_ev_cls(event.EventHostMove)
    def host_moved(self, ev):
        print("Host Moved")

    #@set_ev_cls(event.EventSwitchLeave)
    #def get_topology(self, ev):
    #    print("Swtich Saiu porra!")
